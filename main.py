...
tf.app.flags.DEFINE_string('data_dir',      'yy_data',                  """Path to tfrecords data directory""")
tf.app.flags.DEFINE_string('log_dir',       'checkpoints',           """Path to write logs and checkpoints""")
tf.app.flags.DEFINE_string('images_dir',    'yy_images',                """Path to save generated images""")
tf.app.flags.DEFINE_string('complete_src',  'complete_src',          """Path to images for completion""")
tf.app.flags.DEFINE_string('complete_dir',  'yy_complete',              """Path to save completed images""")
tf.app.flags.DEFINE_string('masktype',      'center',                """Mask types: center, random""") # TODO
tf.app.flags.DEFINE_integer('max_itr',      100001,                  """Maximum number of iterations""")
tf.app.flags.DEFINE_integer('batch_size',   1,                     """Batch size""") # 128
tf.app.flags.DEFINE_integer('latest_ckpt',  0,                       """Latest checkpoint timestamp to load""")
tf.app.flags.DEFINE_integer('nb_channels',  1,                       """Number of color channels""") # 3
...
tf.app.flags.DEFINE_integer('num_examples_per_epoch_for_train', 128, """number of examples for train""") # 300
...
def read_decode(data_dir, batch_size): # s_size
    ...
    image = tf.image.resize_image_with_crop_or_pad(image, 512, 1024)
    ...
    return tf.subtract(tf.div(images, 127.5), 1.0) # s_size * 2 ** 4, s_size * 2 ** 4

def main(_):
    dcgan = DCGAN(batch_size=FLAGS.batch_size, s_size=32, nb_channels=FLAGS.nb_channels) # ssize6
...
if __name__ == '__main__':
    tf.logging.set_verbosity(tf.logging.INFO) # added not tested
    tf.app.run()
